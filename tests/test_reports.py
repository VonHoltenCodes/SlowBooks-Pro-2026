...
def test_pl_income_shows_positive_amount(client, seed_accounts, seed_customer):
    ...
    service = next(
        (r for r in income_rows if r["account_name"] == "Service Income"), None
    )
    assert service is not None
    assert (
        service["amount"] == 100.00
    ), f"income row amount should be +100, got {service['amount']}"
...
def test_ar_aging_buckets_total_correctly(
    ...
    totals = body["totals"]
    buckets_sum = (
        totals["current"] + totals["over_30"] + totals["over_60"] + totals["over_90"]
    )
